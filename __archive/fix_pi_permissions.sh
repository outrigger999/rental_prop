...
# Fix Pi Permissions Script
# Version 1.0
#
# This script fixes permissions for the Moving Box Tracker application on the Raspberry Pi.
# It should be run after sync_to_pi.sh if you experience any permission-related issues.
...
echo -e "${GREEN}[INFO]${NC} Fixing static file permissions on Pi..."

# SSH to the Pi and fix permissions
ssh movingdb << 'EOF'
...
if [ $SSH_EXIT_CODE -ne 0 ]; then
    echo -e "${RED}[ERROR]${NC} Failed to fix permissions on Pi with exit code $SSH_EXIT_CODE"
    exit 1
fi

echo -e "${GREEN}[SUCCESS]${NC} Permissions fixed successfully on Pi."
