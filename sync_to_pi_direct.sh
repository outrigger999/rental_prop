...
# Version 2.0
#
# ENHANCED VERSION: Uses direct rsync with proper permissions to ensure reliable file transfers
...
if [ "$DRY_RUN" = true ]; then
    ...
    ssh $REMOTE_USER@$REMOTE_HOST UPDATE_CONDA=$UPDATE_CONDA TARGET_BRANCH="$TARGET_BRANCH" CONDA_ENV="$CONDA_ENV" << 'EOF'
        ...
        CURRENT_TIMESTAMP="$(date '+%Y-%m-%d %H:%M:%S') (from sync_to_pi_direct.sh)"
...
print_message "Direct sync script completed."
